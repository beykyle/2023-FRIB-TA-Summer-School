...
def initCCsystem(
	...
	level_energies = [0, 2, 4], # energy difference from groud state in each channel [MeV]. This is called $\Delta_n$ in the introductory text.
):
	"""
	...
	for i in range(system.num_channels):
		for j in range(system.num_channels):
			...
			if i == j : # diagonal potentials are just Woods-Saxons
				...
				interaction = lambda r: surface_peaked_gaussian_potential( r, (beta, betaImaginary, R, alpha) )
			else : # off diagonal potential terms, beyond-DWBA
				interaction = lambda r: surface_peaked_gaussian_potential( r, (gamma, gammaImaginary, R, alpha) )
...
if __name__ == "__main__":
	...
	print(timeCCRmatrix(plot=False, V0=60, W0=20, beta=20, gamma=5, collision_energy=13, level_energies=[0, 2, 4]))
